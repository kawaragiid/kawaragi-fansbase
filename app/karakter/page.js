  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white font-sans">
      <div className="container mx-auto px-6 max-w-6xl py-12 pt-28">
        <h1 className="text-5xl font-bold text-center text-white mb-16 border-b-4 border-yellow-300 pb-4 inline-block animate-fade-in-up">Profil Bintang "Monster"</h1>

        {characters.map((char, index) => (
          <section
            key={char.id}
            className={`relative rounded-xl overflow-hidden shadow-2xl mb-24 cursor-pointer group`} // Tambahkan 'group' di sini
            style={{ minHeight: "600px" }}
          >
            {/* Background Image (Parallax-like effect) */}
            <div
              className="absolute inset-0 z-0 bg-fixed bg-center bg-cover opacity-30
                         transition-opacity duration-500 group-hover:opacity-40" // Tambahkan transisi hover untuk opacity background
            ...
            <div className="relative z-10 p-8 md:p-12 lg:p-16 flex flex-col md:flex-row gap-10 items-center justify-center h-full">
              ...
              <div className="flex-grow text-center md:text-left max-w-2xl">
                ...
                >
                  {" "}
                  {/* Tambahkan group-hover untuk judul */}
                  {char.name}
                </h2>
                <p className="text-xl text-yellow-300 font-semibold mb-6 group-hover:text-pink-300 transition-colors duration-300">
                  {" "}
                  {/* Tambahkan group-hover untuk role */}
                  {char.role}
                </p>
                ...
                <ul className="list-disc list-inside text-gray-300 text-lg mb-8 space-y-2">
                  {char.highlights.map((highlight, hIndex) => (
                    <li key={hIndex} className="flex items-center group-hover:text-white transition-colors duration-300">
                      {" "}
                      {/* Tambahkan group-hover untuk list item */}
...
function SocialIconLink({ href, icon, label }) {
  return (
    <Link
      ...
      // Kelas 'group-hover' tidak perlu di sini, karena SocialIconLink memiliki hover-nya sendiri
